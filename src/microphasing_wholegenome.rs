                            ...
                            match variants[j] {
                                ...
                                &Variant::SNV { alt, .. } => {
                                    match variants[j].is_germline() {
                                        true => germline_seq.push(switch_ascii_case(alt, refseq[i as usize])),
                                ...
                                &Variant::Insertion { seq: ref s, .. } => {
                                    match variants[j].is_germline() {
                                        true => germline_seq.extend(switch_ascii_case_vec(s, refseq[i as usize]).into_iter()),
                                        false => indel = true
                                    }
...
//            }
            ...
            let fasta_id = format!("{}", &shaid.digest().to_string()[..15]);
